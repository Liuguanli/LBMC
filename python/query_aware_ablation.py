...
def get_mix_windows(unit_len, dim, ratios, num, bits_nums, query_size, mix_index):
    ...
    for i, ratio in enumerate(ratios):
        temp_windows = get_query_windows(unit_len, dim, ratio, num, bits_nums, query_size)
    ...
    windows_path = 'windows/' + str(dim) + '/mix/'+ str(mix_index) +'/windows.csv'
    # f = open(windows_path, 'w')
    # writer = csv.writer(f)
    # rows = []
    # for window in windows:
...
def quilts_curve_design_multi(windows, frequent_window, dim, bits_nums):
    ...
    d_i = []
    for i in range(dim):
        dim_len = frequent_window.dimension_high[i] - frequent_window.dimension_low[i]
...
def bit_distribution_to_array(bit_distribution, dim, encoding=None):
    res = []
...
def train_optimal_curve_greedy(initial_bit_distribution, bits_nums, windows):
    ...
    while step < iteration:
        ...
        reward = 0
        step += 1
        for action in range(length, -1, -1):
        ...
        if reward == 1:
            continue
        if reward == 0.5:
            exchange, new_bit_distribution = exchange_bit(bit_distribution, exchange_pos)
            bit_distribution = new_bit_distribution
            result_bit_distribution = new_bit_distribution
        if reward <= 0:
            break

    return min_distribution, result_bit_distribution, "", ""

def train_optimal_curve_random(initial_bit_distribution, bits_nums, windows):
    length = sum(bits_nums)
    dim = len(bits_nums)
    bit_distribution = initial_bit_distribution
    GC = GlobalCost(windows, bits_nums)
    ...
    for i in range(M):
        ...
        while step < iteration:
            ...
            state = bit_distribution_to_array(bit_distribution, dim)
            # print(state)
            action = np.random.randint(0, length)
...
def train_optimal_curve(initial_bit_distribution, bits_nums, windows, encoding=None):
    length = sum(bits_nums)
    dim = len(bits_nums)
    bit_distribution = initial_bit_distribution
    if encoding is None:
    ...
    for i in range(M):
        ...
        while step < iteration:
            # print(bit_distribution)
            state = bit_distribution_to_array(bit_distribution, dim, encoding)
            # print(state)
            ...
            # print(reward)
            current_state = bit_distribution_to_array(new_bit_distribution, dim, encoding)
...
def train_optimal_curve_vary_cost(initial_bit_distribution, bits_nums, windows, cost_type):
    length = sum(bits_nums)
    dim = len(bits_nums)
    bit_distribution = initial_bit_distribution
...
def get_bit_dis_comb(res, current, bit_nums, total_length):
    if (len(current) == total_length):
        ...
        for i in range(len(bit_nums)):
            if bit_nums[i] > 0:
                bit_nums[i] -= 1
                get_bit_dis_comb(res, current+bit_letters[i], bit_nums, total_length)
                bit_nums[i] += 1
...
def test_pattern(dim, num, ratios, unit_len, bits_nums, index):
    for ratio in ratios:
        json_path = 'windows/' + \
            str(dim) + "/" + str(index) + "/" + str(num) + "/" + ratio_to_pattern(ratio) + '/bit_distributions.json'
        print(json_path)
        # continue
...
def random_bit_distribution_C(old_bit_nums=[8, 8]):
    bit_nums = copy.deepcopy(old_bit_nums)
    res = ""
    all_ = sum(old_bit_nums)
    i = all_
    dim = len(bit_nums)
    for index, num in enumerate(bit_nums):
        for i in range(num):
            res += bit_letters[index]
    return res

def random_bit_distribution_random(old_bit_nums=[8, 8]):
    bit_nums = copy.deepcopy(old_bit_nums)
    res = ""
    all_ = sum(old_bit_nums)
    i = all_
    dim = len(bit_nums)
    while len(res) != all_:
        action = np.random.randint(0, dim)
        if bit_nums[action] == 0:
                continue
        else:
            res = res + bit_letters[action]
            bit_nums[action] -= 1
    return res


def test_mix(unit_len, dim, ratios, num, bits_nums, unit_len_scale, mix_index, ab_item, type):
    json_path = 'ablation/' + ab_item + "/" + type + '/' + str(mix_index) + '/bit_distributions.json'
    print(json_path)
    windows = get_mix_windows(unit_len, dim, ratios, num, bits_nums, unit_len_scale, mix_index)

    init_bit_distribution = random_bit_distribution(bits_nums)
    quilts_bit_distribution = quilts_curve_design_multi(windows, windows[0], dim, bits_nums)

    if ab_item == "exchange":
        if type == "random":
            min_distribution, rl_bit_distribution, global_bit_distribution, local_bit_distribution = train_optimal_curve_random(
                init_bit_distribution, bits_nums, windows)
        elif type == "greedy":
            min_distribution, rl_bit_distribution, global_bit_distribution, local_bit_distribution = train_optimal_curve_greedy(
                init_bit_distribution, bits_nums, windows)
        else:
            min_distribution, rl_bit_distribution, global_bit_distribution, local_bit_distribution = train_optimal_curve(
            init_bit_distribution, bits_nums, windows)
        ...
        if type == "C":
            init_bit_distribution = random_bit_distribution_C(bits_nums)
        elif type == "random":
            init_bit_distribution = random_bit_distribution_random(bits_nums)
        elif type == "QUILTS":
            init_bit_distribution = quilts_bit_distribution[1]
        else:
            init_bit_distribution = random_bit_distribution(bits_nums)
        min_distribution, rl_bit_distribution, global_bit_distribution, local_bit_distribution = train_optimal_curve(
            init_bit_distribution, bits_nums, windows)
    elif ab_item == "encoding":
        min_distribution, rl_bit_distribution, global_bit_distribution, local_bit_distribution = train_optimal_curve(
            init_bit_distribution, bits_nums, windows, type)
    elif ab_item == "cost":
        min_distribution, rl_bit_distribution, global_bit_distribution, local_bit_distribution = train_optimal_curve_vary_cost(
            init_bit_distribution, bits_nums, windows, type)
    else:
        pass
    
    data = {"rld": rl_bit_distribution, "quilts_Z": quilts_bit_distribution[1]}
    with open(json_path, 'w') as outfile:
        json.dump(data, outfile)


def test_2d_mix(ab_item, type):
    delta = 0.004
    ...
    test_mix(unit_len, dim, ratios, query_num, bits_nums, 1, 1, ab_item, type)
    ratios = [[1.0, 256.0], [2, 128], [4, 64], [8, 32], [16, 16]]
    test_mix(unit_len, dim, ratios, query_num, bits_nums, 1, 2, ab_item, type)
    ratios = [[128.0, 2.0],[256.0, 1.0],[4, 64], [1.0, 256.0],  [2, 128],  [64, 4]]
    test_mix(unit_len, dim, ratios, query_num, bits_nums, 1, 3, ab_item, type)


def test_ablation():
    inits = ["random", "C", "Z", "QUILTS"]
    encodings = ["no", "normalized", "one_hot"]
    exchanges = ["random", "greedy"]
    cost = ["global", "local", "total"]
    ablation_config = {"init":inits, "encoding":encodings, "exchange":exchanges, "cost":cost}

    for key in ablation_config.keys():
        vals = ablation_config[key]
        for val in vals:
            print("------------------", key, ":", val, "------------------")
            test_2d_mix(key, val)
