...
class MCTS:
    ...
    def do_rollout(self, tree, env, original_query):  # in our LSFC problem can limit a length of the path
                ...
                for mask in masks:
                    ...
                    for i in range(len(mask)):
                        if mask[i] == 1:
                            ...
                            # get the performance
                            if not env.is_opt_cost:
                                env._order_generate()
                            value_range = env.fast_compute_scan_range(query)
                            # value_range = env.optimised_fast_compute_scan_range(query)
                            # value_range = env.module.output_all_queries_global(query) / len(query)
                            
                            greed_perf[i] = value_range
    ...
    def _simulate(self, tree, env, query):
        ...
        # scan_range = env.optimised_fast_compute_scan_range(query)
        reward = self.initial_scanrange - scan_range
        # end_time = time.time()
        # print("_simulate time cost: ", end_time - start_time)
        return reward
