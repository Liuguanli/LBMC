...
val tableName = if (layoutOptStrategy == "bmc") {
  s"${dataFileName}_${layoutOptStrategy}_returnflag_linestatus_${bmcPattern}"
...
def writeHudiTable(df: DataFrame,
  ...
  if (layoutOptStrategy == "default") {
    ...
    .option(HoodieClusteringConfig.PLAN_STRATEGY_SORT_COLUMNS.key, "l_commitdate,l_receiptdate,l_shipdate")
    .option(DataSourceWriteOptions.OPERATION.key(), DataSourceWriteOptions.BULK_INSERT_OPERATION_OPT_VAL)
    .option(BULK_INSERT_SORT_MODE.key(), "NONE")
    .options(commonOpts)
    //  .mode(ErrorIfExists)
...
val outputFile = if (layoutOptStrategy == "bmc") {
  s"./hudi/result/${dataFileName}_${layoutOptStrategy}_3d_${bmcPattern}.json"
} else {
  s"./hudi/result/${dataFileName}_${layoutOptStrategy}_3d_${bytesNum}.json"
}
