...
def writeHudiTable(df: DataFrame,
  ...
  if (layoutOptStrategy == "default") {
    ...
    .option(HoodieClusteringConfig.PLAN_STRATEGY_SORT_COLUMNS.key, "l_commitdate,l_receiptdate,l_shipdate,l_linenumber,l_quantity,l_discount")
    .option(DataSourceWriteOptions.OPERATION.key(), DataSourceWriteOptions.BULK_INSERT_OPERATION_OPT_VAL)
    .option(BULK_INSERT_SORT_MODE.key(), "NONE")
    .options(commonOpts)
    //  .mode(ErrorIfExists)
...
val outputFile = if (layoutOptStrategy == "bmc") {
  s"./hudi/result/${dataFileName}_${layoutOptStrategy}_6d.json"
} else {
  s"./hudi/result/${dataFileName}_${layoutOptStrategy}_6d_${bytesNum}.json"
}
...
runQuery("Q5", true,
      ...
      val discount_min = random.nextDouble() * 0.1
